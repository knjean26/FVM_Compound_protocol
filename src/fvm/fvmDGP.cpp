...
void FvmDGP::initDataSchedule(){
    dataSchedule = scheduleDataForBlockNumber(0);
}

bool FvmDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData, int blockHeight){
    const Consensus::Params& consensusParams = Params().GetConsensus();

    if(defaultData.size() == 39 && (checkData.size() == 39 || (checkData.size() == 40 && blockHeight > 0))) {
...
dev::eth::EVMSchedule FvmDGP::getGasSchedule(int blockHeight){
    clear();
...
uint64_t FvmDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t FvmDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t FvmDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t FvmDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
void FvmDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void FvmDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void FvmDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void FvmDGP::createParamsInstance(){
...
dev::Address FvmDGP::getAddressForBlock(unsigned int blockHeight){
...
void FvmDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void FvmDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void FvmDGP::parseStorageOneUint64(uint64_t& value){
...
void FvmDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = dev::toUint64(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule FvmDGP::createEVMSchedule(const dev::eth::EVMSchedule &_schedule, int blockHeight){
