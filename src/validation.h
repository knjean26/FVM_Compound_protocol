...
using ExtractFvmTX = std::pair<std::vector<FvmTransaction>, std::vector<EthTransactionParams>>;
...
static const uint64_t DEFAULT_GAS_LIMIT_OP_CREATE=2500000*1000000000000000;
static const uint64_t DEFAULT_GAS_LIMIT_OP_SEND=250000*1000000000000000;
...
class FvmTxConverter{

public:

    FvmTxConverter(CTransaction tx, CCoinsViewCache* v = NULL, const std::vector<CTransactionRef>* blockTxs = NULL, unsigned int flags = SCRIPT_EXEC_BYTE_CODE) : txBit(tx), view(v), blockTransactions(blockTxs), sender(false), nFlags(flags){}

    bool extractionFvmTransactions(ExtractFvmTX& fvmTx);
...
private:
    ...
    FvmTransaction createEthTX(const EthTransactionParams& etp, const uint32_t nOut);
...
public:
    ...
    ByteCodeExec(const CBlock& _block, std::vector<FvmTransaction> _txs, const uint64_t _blockGasLimit, CBlockIndex* _pindex) : txs(_txs), block(_block), blockGasLimit(_blockGasLimit), pindex(_pindex) {}
